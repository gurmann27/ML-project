...
export function ChurnPage() {
  ...
  async function run() {
    ...
    setLoading(true);
    try {
      const res = await apiPost<unknown>("/api/v1/churn/predict", body);
      setOut(res);
    } catch (e) {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        <h2 className="font-display text-2xl text-ink-950">Churn prediction</h2>
        <p className="text-slate-600 mt-1">
          Binary classification with risk bands and SHAP-style factor explanations (XGBoost or demo).
        </p>
      </div>
