...
import { highRiskCustomer } from "../lib/sampleCustomer";
import { CustomerJsonPanel } from "../components/CustomerJsonPanel";

export function AnomalyPage() {
  const [json, setJson] = useState(JSON.stringify(highRiskCustomer, null, 2));
  ...
  async function run() {
    ...
    setLoading(true);
    try {
      const res = await apiPost<unknown>("/api/v1/anomaly/detect", body);
      setOut(res);
    } catch (e) {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div>
        <h2 className="font-display text-2xl text-ink-950">Anomaly detection</h2>
        <p className="text-slate-600 mt-1">Isolation Forest–style scoring with rule-based flags.</p>
      </div>
